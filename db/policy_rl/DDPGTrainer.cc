...
DDPGTrainer::DDPGTrainer(int64_t input_channels, int64_t num_actions, int64_t capacity) {
    ...
    actor_optimizer =  torch::optim::Adam(actor_local->parameters(), lr_actor);
  
    critic_local = std::make_shared<Critic>(stateSize, actionSize);
    critic_target = std::make_shared<Critic>(stateSize, actionSize);
    critic_optimizer =  torch::optim::Adam(critic_local->parameters(), lr_critic);
...
/******************* ACTOR *******************/
DDPGTrainer::Actor::Actor(int64_t state_size, int64_t action_size,
...
std::pair<double,double> DDPGTrainer::Actor::hidden_init(torch::nn::Linear& layer) {
  double lim = 1. / sqrt(layer->weight.sizes()[0]);
  return std::make_pair(-lim, lim);
}

void DDPGTrainer::Actor::reset_parameters() {
...
torch::Tensor DDPGTrainer::Actor::forward(torch::Tensor x) {
...
torch::nn::BatchNormOptions DDPGTrainer::Actor::bn_options(int64_t features) {
...
/******************* Critic *****************/
DDPGTrainer::Critic::Critic(int64_t state_size, int64_t action_size,
...
std::pair<double,double> DDPGTrainer::Critic::hidden_init(torch::nn::Linear& layer) {
  double lim = 1. / sqrt(layer->weight.sizes()[0]);
  return std::make_pair(-lim, lim);
}

void DDPGTrainer::Critic::reset_parameters() {
...
torch::Tensor DDPGTrainer::Critic::forward(torch::Tensor x, torch::Tensor action) {
  if (x.dim() == 1)
